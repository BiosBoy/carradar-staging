...
import { toggleDarkMode } from '../../controller/actions';
...
const DarkModeSwitcher = () => {
  ...
  const { isDarkMode } = useSelector(({ app }: IStore) => app);
  ...
  return (
    <div
      ...
      role='button'
      aria-label='Dark and Light mode switcher'
      tabIndex={0}
      className={`${styles.toggle} ${isDarkMode ? styles.toggleChecked : ''}`}
      onKeyDown={undefined}
      onClick={_handleDarkModetoggle}
    >
      <div className={styles.toggleTrack}>
        <div className={styles.toggleTrackCheck}>
          <img alt='' src={ICON_SUN} />
        </div>
        <div className={styles.toggleTrackX}>
          <img alt='' src={ICON_MOON} />
        </div>
      </div>
      <div className={styles.toggleThumb} />
      <input className={styles.toggleScreenreaderOnly} type='checkbox' />
    </div>
  );
};
